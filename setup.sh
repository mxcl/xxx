...
is_complex_install_target() {
  case "$1" in
    aws|clawhub|node|openclaw)
      return 0
      ;;
    *)
      return 1
...
queue_brew_upgrade() {
  queue_command "brew upgrade"
}

queue_uv_python_upgrade() {
  queue_command "uv python upgrade"
}

queue_rustup_upgrade() {
  queue_command "rustup update"
}

simple_install_repo_for_target() {
  case "$1" in
    brewx)
      printf '%s\n' "mxcl/brewx"
      ;;
...
queue_simple_install_command() {
  name="$1"
  version="$2"

  if [ "${name}" = "yoink" ]; then
    yoink_spec="mxcl/yoink"
    if [ "${version}" != "latest" ]; then
      yoink_spec="${yoink_spec}@${version}"
    fi
    queue_command "curl -fsSL https://yoink.sh | sh -s -- ${yoink_spec}"
    queue_command "sudo install -m 755 -D ./yoink /usr/local/bin/yoink"
    return 0
  fi

  repo="$(simple_install_repo_for_target "${name}" || true)"
  if [ -z "${repo}" ]; then
    return 1
  fi
  ...
  queue_command \
    "yoink ${install_spec} | xargs -I{} sudo mv {} /usr/local/bin"
  return 0
}
...
queue_install() {
  ...
  if is_complex_install_target "${name}"; then
    queue_apply_arg "--install"
    queue_apply_arg "${name}=${version}"
    return 0
  fi

  if queue_simple_install_command "${name}" "${version}"; then
    return 0
  fi
...
emit_plan() {
  ...
  if [ -n "${PLAN_COMMANDS}" ]; then
    printf '%s\n' "${PLAN_COMMANDS}"
  fi

  if [ -n "${APPLY_ARGS}" ]; then
    printf '/usr/local/bin/outdated --apply%s\n' "${APPLY_ARGS}"
  fi
}
...
$1"
  ...
  if [ -n "${INSTALL_QUEUE}" ]; then
    ...
    while IFS= read -r spec; do
      ...
      if ! is_complex_install_target "${name}"; then
        echo "panic: apply only supports complex install target: ${name}" >&2
        exit 1
      fi
...
/usr/local/bin/npm install -g --prefix "${PWD}" openclaw@${1:-latest}

# Upstream artifacts include 0600 files; normalize so non-root users can run.
chmod -R u+rwX,go+rX ./lib/node_modules/openclaw

$_SUDO rm -f /usr/local/bin/openclaw
$_SUDO rm -rf /usr/local/lib/node_modules/openclaw
$_SUDO install -d -m 755 /usr/local/bin /usr/local/lib/node_modules
$_SUDO cp -RP ./lib/node_modules/openclaw /usr/local/lib/node_modules/openclaw
$_SUDO cp -RP ./bin/openclaw /usr/local/bin/openclaw
}
...
/usr/local/bin/npm view clawhub version
}
